...
export const Navigation = () => {
  ...
  const [currentWidth, setCurrentWidth] = useState<number>(240);
  ...
  const open = () => {
    if (sidebarRef.current && navbarRef.current) {
      ...
      sidebarRef.current.style.width = isMobile ? "100%" : `${currentWidth}px`;
      navbarRef.current.style.setProperty(
        "width",
        isMobile ? "100%" : `${currentWidth}px`,
      );
      navbarRef.current.style.setProperty(
        "left",
        isMobile ? "100%" : `${currentWidth}px`,
  ...
  return (
    <>
      <aside
        ref={sidebarRef}
        className={cn(
          "group/sidebar h-full bg-secondary overflow-y-auto relative flex w-60 flex-col z-[99999] py-2",
      ...
      >
        ...
        <div
          ...
          onClick={resetWidth}
          className="opacity-0 hover:opacity-100 transition cursor-ew-resize absolute h-full w-1 bg-primary/5 right-0 top-0"
        />
      </aside>
      <div
        ref={navbarRef}
